...
function Projects() {
  ...
  return (
    <section id="projects" className="relative py-24">
      <div className="max-w-7xl mx-auto px-6">
        <SectionHeader tag="// Featured Work" title={<>Selected <span className="neon-text">Projects</span></>} subtitle="A snapshot of what I've shipped while learning AI, ML and data science. Click any card to dive in." />
          ...
          {filtered.map((p) => (
            <button key={p.title} onClick={() => setOpen(p)}
              ...
              <div className="flex items-start justify-between mb-4 relative">
                <div className="w-11 h-11 rounded-lg bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-up" onClick={() => setOpen(null)}>
          <div className="absolute inset-0 bg-background/80 backdrop-blur-md" />
          <div className="relative glass-card border-primary/40 max-w-2xl w-full p-8 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setOpen(null)} className="absolute top-4 right-4 w-9 h-9 rounded-full glass-card flex items-center justify-center hover:text-primary">
              <X className="w-4 h-4" />
            </button>
            <span className="chip mb-4">{open.cat}</span>
            <h3 className="text-3xl font-bold mb-3">{open.title}</h3>
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-6">
              <div className="rounded-lg border border-border bg-secondary/30 p-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {open.tech.map((t) => (
                    <span key={t} className="text-[11px] px-2 py-0.5 rounded-md bg-primary/10 border border-primary/30 text-primary font-mono">{t}</span>
                  ))}
                </div>
              </div>
              <div className="rounded-lg border border-border bg-secondary/30 p-4">
                <div className="text-xs font-mono text-muted-foreground mb-2">// Highlights</div>
            ...
            <a href={open.link} target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-primary text-primary-foreground font-semibold neon-glow hover:scale-105 transition-transform">
